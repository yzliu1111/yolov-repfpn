...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy, bbox_iou_eciou, bbox_iou_xiou
...
class PolyFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        if self.reduction == 'mean':
            return polyloss.mean()
        elif self.reduction == 'sum':
            return polyloss.sum()
        else:  # 'none'
            return polyloss

#原版computeloss，已经加入了GFL,QFL,VFL,polyloss以及focal siou/eiou等的改造(专栏中上述两种改进都默认用computeloss)
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False):
        ...
        # 使用qualityfocalloss
        '''if h['qualityfl']==True:
            g = h['qualityfl_gamma']  # qualityfloss
            if g > 0:
        ...
        # 使用polyloss
        if h['polyloss']==True:
            g = h['polyl_gamma']  # polyloss
            if g > 0:
                BCEcls, BCEobj = PolyFocalLoss(BCEcls, g), PolyFocalLoss(BCEobj, g)'''
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #改进focal siou/ciou/diou/eiou的bbox_iou，FocalLoss_和True的类别要一样
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, FocalLoss_ ='SIoU', EIoU=True)  # iou(prediction, target)
                iou_ori = iou[0]
                iou_f = iou[1]
                if len(iou) >= 2:
...
#原版computelossota，已经加入了GFL,QFL,VFL,polyloss以及focal siou/eiou等的改造(专栏中上述两种改进都默认用computeloss)
#还加入了eciou(ota专属)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                #eciou
                #iou = bbox_iou_eciou(pbox.T, selected_tbox, x1y1x2y2=False, EfficiCIoULoss=True)  


                #改进focal siou/ciou/diou/eiou的bbox_iou，FocalLoss_和True的类别要一样,注意形式都是SIoU，o小写
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, FocalLoss_ ='SIoU', SIoU=True)  # iou(prediction, target)
                iou_ori = iou[0]
                iou_f = iou[1]
                if len(iou) >= 2:
                    lbox += (iou_f.detach() * (1 - iou_ori)).mean()
                ...
                #xiou
                #iou = bbox_iou_xiou(pbox.T, selected_tbox, x1y1x2y2=False, XIoU=True)  
